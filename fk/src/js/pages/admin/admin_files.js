...
const AdminFiles = (props) => {
        ...
        // },        
        {
            ...
            render: (_, record) => {
                return <Space size="middle">
                    <Button className='btn-successx border-0x' onClick={e => editRecord(record, 'admin_files')}><i className='fas fa-edit text-success' /></Button>
                    {del.confirm(
                        `${Settings.backend}/delete`,
                        record,
                        'Are you sure to delete this item',
                        { tableName: 'admin_files', where: 'id', whereType: 'closed' },
    ...
    function addRecord(tableName = 'admin_files') {
    ...
    async function editOnOk() {
        let res = await edit.save(undefined,
            ...
            'admin_files',
            { tbl: 'admin_files', where: 'id', whereType: 'closed' },
        );
    }

    async function addOnOk() {
        let res = await add.save(`${Settings.backend}/add`, { tbl: 'admin_files' })
    }
...
export default memo(AdminFiles);
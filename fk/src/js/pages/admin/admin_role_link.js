...
const AdminRoleLink = (props) => {
    ...
    const table = useTable(
        ...
        `${Settings.backend}/get_admin_role_link`,
        'post',
        'result',
        'totalCount',
        'id',
        { /*alias: 'LIKE', acadyr: 'LIKE', semester: 'IN', end_date: 'IN', is_active: 'IN' */ },
        { table: 'admin_role_link', fields: ['*'] });
        ...
        // },        
        {
            ...
            render: (_, record) => {
                return <Space size="middle">
                    <Button className='btn-successx border-0x' onClick={e => editRecord(record, 'admin_role_link')}><i className='fas fa-edit text-success' /></Button>
                    {del.confirm(
                        `${Settings.backend}/delete`,
                        record,
                        'Are you sure to delete this item',
                        { tableName: 'admin_role_link', where: 'id', whereType: 'closed' },
    ...
    function addRecord(tableName = 'admin_role_link') {
    ...
    async function editOnOk() {
        let res = await edit.save(undefined,
            ...
            'admin_role_link',
            { tbl: 'admin_role_link', where: 'id', whereType: 'closed' },
        );
    }

    async function addOnOk() {
        let res = await add.save(`${Settings.backend}/add`, { tbl: 'admin_role_link' })
    }
...
export default memo(AdminRoleLink);
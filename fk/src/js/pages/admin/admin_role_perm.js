...
const AdminRoles = (props) => {
    ...
    const columns = ([
        ...
        },
        {
            ...
            ...table.getColumnSearchProps('permission_id'),
            // render: (v, record) => {
        ...
        // },        
        {
            ...
            render: (_, record) => {
                return <Space size="middle">
                    <Button className='btn-successx border-0x' onClick={e => editRecord(record, 'admin_role_perm')}><i className='fas fa-edit text-success' /></Button>
                    {del.confirm(
                        `${Settings.backend}/delete`,
                        record,
                        'Are you sure to delete this item',
                        { tableName: 'admin_role_perm', where: 'id', whereType: 'closed' },
    ...
    function addRecord(tableName = 'admin_role_perm') {
    ...
    async function editOnOk() {
        let res = await edit.save(undefined,
            ...
            'admin_role_perm',
            { tbl: 'admin_role_perm', where: 'id', whereType: 'closed' },
        );
    }

    async function addOnOk() {
        let res = await add.save(`${Settings.backend}/add`, { tbl: 'admin_role_perm' })
    }

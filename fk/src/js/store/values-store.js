...
const store = (set, get) => ({
    ...
    permitted_routes: [],//admin/home.js,
    ...
    getArrayObjectsValue: (stateKey, searchKey, value) => {//returns a single item
    ...
    getValuesBy: (stateKey, searchKey, value) => {//returns an array 
        const state = get();//zustand get callback function
        const record = state[stateKey];
        let val = [];
        if (Array.isArray(record)) {
    ...
    updateArrayObjectValue: (stateKey, whereKey, whereValue, itemKey, newValue) => {
        set((state) => {
            const record = state[stateKey];
            let newState = [...record];
            newState && newState?.forEach((v, i) => {
